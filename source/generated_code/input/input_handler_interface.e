...
feature -- parsing
	...
	evt_to_cmd (evt : TUPLE[name: STRING; args: ARRAY[EVT_ARG]]) : COMMAND_INTERFACE
		...
		do
			...
			elseif cmd_name ~ "add_type" then 
				 if attached {STR_ARG} args[1] as product_id then 
			...
			elseif cmd_name ~ "add_product" then 
				 if attached {STR_ARG} args[1] as a_product and then attached {INT_ARG} args[2] as quantity then 
			...
			elseif cmd_name ~ "add_order" then 
				 if (attached {ARRAY_ARG} args[1] as a_order and then across a_order.value as member all attached {TUPLE_ARG} member.item as tup  and then tup.value.count = 2 and then (attached {STR_ARG} tup.value[1] as pid) and then (attached {INT_ARG} tup.value[2] as no) end) then 
			...
			elseif cmd_name ~ "invoice" then 
				 if attached {INT_ARG} args[1] as order_id then 
	...
	do
		...
		loop
			...
			elseif cmd_name ~ "add_type" then 
				if NOT( ( args.count = 1 ) AND THEN attached {STR_ARG} args[1] as product_id) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
			...
			elseif cmd_name ~ "add_product" then 
				if NOT( ( args.count = 2 ) AND THEN attached {STR_ARG} args[1] as a_product and then attached {INT_ARG} args[2] as quantity) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
			...
			elseif cmd_name ~ "add_order" then 
				if NOT( ( args.count = 1 ) AND THEN (attached {ARRAY_ARG} args[1] as a_order and then across a_order.value as member all attached {TUPLE_ARG} member.item as tup  and then tup.value.count = 2 and then (attached {STR_ARG} tup.value[1] as pid) and then (attached {INT_ARG} tup.value[2] as no) end)) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
			...
			elseif cmd_name ~ "invoice" then 
				if NOT( ( args.count = 1 ) AND THEN attached {INT_ARG} args[1] as order_id) then 
					if NOT Result.is_empty then
						Result.append ("%N")
					end
					Result.append (evt_out_str + " does not conform to declaration " +
